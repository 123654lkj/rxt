...
pub fn run(
    ...
    if let Some(remote) = remote {
        return run_remote(pattern, path, context, only_count, invert, json_output, remote);
    }
    ...
    let all_results: Vec<Match> = if path.is_dir() {
        ...
        files.par_iter()
            .filter_map(|p| {
                ...
                let results = search_in_content(&content, &compiled_re, &pattern_lower, context, invert, &display);
        ...
        let sig = FileSignature::detect(&raw);
        let content = to_utf8_lf(&raw, &sig);
        search_in_content(&content, &compiled_re, &pattern_lower, context, invert, &path.to_string_lossy())
    };
    ...
    if offset > 0 {
        if offset >= all_results.len() { all_results.clear(); }
        else { all_results = all_results.split_off(offset); }
    }
    if let Some(h) = head { all_results.truncate(h); }
    if let Some(m) = max_results { all_results.truncate(m); }
    ...
    if jsonl {
        for (f, ln, txt, _) in &all_results {
            writeln!(out, "{}", serde_json::json!({"path": f, "line": ln, "text": txt}))?;
        }
    } else if json_output {
        let j: Vec<_> = all_results.iter().map(|(f, ln, txt, _)|
            serde_json::json!({"path": f, "line": ln, "text": txt})).collect();
        ...
        for (f, _ln, _txt, ctx) in &all_results {
            if *f != current_file { writeln!(out, "\n{}:", f)?; current_file = f.clone(); }
...
fn run_remote(
    ...
    if json_output {
        let results: Vec<_> = output.lines().filter_map(|line| {
        ...
        for line in output.lines() {
            ...
            if parts.len() == 2 {
                if parts[0] != current_file { println!("\n{}:", parts[0]); current_file = parts[0].to_string(); }
                println!("{}", parts[1]);
            }
        }
