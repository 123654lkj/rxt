...
pub fn run(cmd: &str, paths: &[String], exts: &str, debounce_ms: u64, run_on_start: bool) -> anyhow::Result<()> {
    ...
    let ext_filter: Vec<String> = exts.split(',').filter(|s| !s.trim().is_empty())
        .map(|s| s.trim().trim_start_matches('.').to_lowercase()).collect();

    let (tx, rx) = std::sync::mpsc::channel::<notify::Result<Event>>();
    let mut watcher = notify::RecommendedWatcher::new(move |res| { let _ = tx.send(res); }, NotifyConfig::default())?;

    for p in &watch_paths {
        watcher.watch(p, RecursiveMode::Recursive)?;
    }
    println!("👀 watching: {} (exts: [{}], debounce: {}ms)",
        watch_paths.iter().map(|p| p.display().to_string()).collect::<Vec<_>>().join(", "),
        if ext_filter.is_empty() {"all".into()} else {ext_filter.join(",")},
        debounce_ms);
    println!("▶ cmd: {}\n", cmd);
    ...
    let run_cmd = || {
        ...
        match status {
            Ok(s) => {
                let mark = if s.success() { "✓".to_string() } else { format!("✗ exit {}", s.code().unwrap_or(-1)) };
                println!("\n⏱ {:.2}s {} (waiting for changes...)\n", elapsed.as_secs_f64(), mark);
            }
            Err(e) => eprintln!("\n✗ 启动失败: {}\n", e),
        }
    };

    if run_on_start { run_cmd(); }
