...
use crate::signature::{apply_format, to_utf8_lf, Encoding, FileSignature, LineEnding};

/// 统一文件格式
pub fn run(
    path: &Path,
    target_ending: Option<&str>,
    remove_bom: bool,
    json_output: bool,
) -> anyhow::Result<()> {
    ...
    let target_le = match target_ending {
        ...
        _ => anyhow::bail!(
            "Unknown line ending: {}. Use: lf, crlf, unix, windows",
            target_ending.unwrap()
        ),
    };
