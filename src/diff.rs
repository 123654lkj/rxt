...
pub fn run(
    ...
    match second {
        ...
        Some(s) if first.is_dir() && s.is_dir() => dir_diff(
            first,
            s,
            context,
            only_stat,
        ...
        Some(s) if first.is_file() && s.is_file() => file_diff(
            first,
            s,
            context,
            only_stat,
...
fn git_diff(
    ...
    let output = std::process::Command::new("git")
        .args(["diff", &format!("-U{}", context)])
        .output()
        .map_err(|e| anyhow::anyhow!("git not available: {}", e))?;
    if !output.status.success() {
        eprintln!("  (no git repo or no changes)");
        return Ok(());
    }
    ...
    if only_stat {
        ...
        println!(
            ...
            serde_json::to_string_pretty(
                &serde_json::json!({"type":"git","files":files.len(),"raw":stdout})
            )?
        );
    } else {
        println!("{}", stdout);
...
fn file_diff(
    ...
    if json_output {
        return file_diff_json(
            first, second, &a_lines, &b_lines, &changes, context, adds, dels,
        );
    }
    if ai_mode {
    ...
    let change_idx: Vec<usize> = changes
        .iter()
        .enumerate()
        .filter(|(_, c)| c.typ != 'e')
        .map(|(i, _)| i)
        ...
        while i < ctx_hi {
            ...
            match ch.typ {
                'e' => {
                    ...
                    write_side_by_side_line(
                        &mut out, ' ', ch.old_idx, line, ch.new_idx, ' ', line, left_w, right_w,
                    )?;
                    i += 1;
                }
                'd' => {
                        ...
                        for j in 0..n {
                            ...
                            match (left, right) {
                                (Some((la, ll)), Some((rb, rl))) => {
                                    write_side_by_side_line(
                                        &mut out, '-', la, ll, rb, '+', rl, left_w, right_w,
                                    )?;
                                }
                                (Some((la, ll)), None) => {
                                    write_side_by_side_line(
                                        &mut out, '-', la, ll, 0, ' ', "", left_w, right_w,
                                    )?;
                                }
                                (None, Some((rb, rl))) => {
                                    write_side_by_side_line(
                                        &mut out, ' ', 0, "", rb, '+', rl, left_w, right_w,
                                    )?;
                                }
                                _ => {}
                            }
...
fn write_side_by_side_line(
    ...
    let l_num_str = if l_num > 0 {
        format!("{:>4}", l_num)
    } else {
        "    ".to_string()
    };
    let r_num_str = if r_num > 0 {
        format!("{:>4}", r_num)
    } else {
        "    ".to_string()
    };
