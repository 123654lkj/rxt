...
pub fn run(patterns: &[String], cmd: &str, path: Option<&Path>, debounce_ms: u64) -> anyhow::Result<()> {
    let (tx, rx) = mpsc::channel::<()>();
    let pats: Vec<String> = patterns.to_vec();
    let watch_dir = path.map(|p| p.to_path_buf()).unwrap_or_else(|| std::env::current_dir().unwrap());
            ...
            move |res: Result<Event, notify::Error>| {
                if let Ok(event) = res {
                    for ev_path in &event.paths {
                        ...
                        let matched = pats.is_empty() || pats.iter().any(|p| {
                            if p.starts_with("*.") {
                                pname.ends_with(&p[1..])
                            } else {
    ...
    for _ in rx {
        ...
        if cfg!(windows) { command.arg("/C"); } else { command.arg("-c"); }
