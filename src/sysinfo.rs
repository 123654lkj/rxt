...
pub fn run(section: &str, json_output: bool) -> anyhow::Result<()> {
    ...
    match section {
        ...
        "disk" => print_disk(),
        "net" => print_net(),
        other => anyhow::bail!("unknown section '{}', valid: all os cpu mem disk net", other),
    }
    Ok(())
}
...
fn print_os(sys: &System) {
    println!("== OS / 主机 ==");
    println!("  系统:   {} {}", System::name().unwrap_or_default(), System::long_os_version().unwrap_or_default());
...
fn print_cpu(sys: &System) {
    ...
    let total_usage: f32 = cpus.iter().map(|c| c.cpu_usage()).sum::<f32>() / cpus.len().max(1) as f32;
    println!("  总使用: {:.1}%", total_usage);
    if cpus.len() > 1 {
        let per: Vec<String> = cpus.iter().take(16).map(|c| format!("{:.0}", c.cpu_usage())).collect();
        println!("  各核:   {}%", per.join("/"));
    }
}
...
fn print_mem(sys: &System) {
    ...
    println!("  已用:   {} ({:.0}%)", format_bytes(sys.used_memory()),
             pct(sys.used_memory(), sys.total_memory()));
    if sys.total_swap() > 0 {
        println!("  交换:   {}/{} ({:.0}%)", format_bytes(sys.used_swap()), format_bytes(sys.total_swap()),
...
fn print_disk() {
    ...
    for d in disks.list() {
        ...
        println!("  {:<14} {:>10} / {:>10} ({:>5.0}%)  {}",
                 &name.to_string().chars().take(14).collect::<String>(),
                 format_bytes(used), format_bytes(total), pct(used, total), mp);
        shown += 1;
    }
    if shown == 0 { println!("  (无)"); }
}
...
fn print_net() {
    ...
    for (name, data) in nets.list() {
        println!("  {:<16} ↓{}/s  ↑{}/s", name, format_bytes(data.received()), format_bytes(data.transmitted()));
        shown += 1;
    }
    if shown == 0 { println!("  (无)"); }
}
...
fn build_json(sys: &System, section: &str) -> anyhow::Result<serde_json::Value> {
    let cpus = sys.cpus();
    let cpu_arr: Vec<serde_json::Value> = cpus.iter().map(|c| serde_json::json!({
        "name": c.name(),
        "brand": c.brand(),
    ...
    let disk_arr: Vec<serde_json::Value> = Disks::new_with_refreshed_list().list().iter().map(|d| {
        let total = d.total_space();
        let used = total - d.available_space();
        serde_json::json!({
    ...
    let net_arr: Vec<serde_json::Value> = Networks::new_with_refreshed_list().list().iter().map(|(n, d)| serde_json::json!({
...
fn format_bytes(b: u64) -> String {
    ...
    if b >= TB { format!("{:.1}T", b as f64 / TB as f64) }
    else if b >= GB { format!("{:.1}G", b as f64 / GB as f64) }
