...
pub fn run(repo: Option<&str>, no_deploy: bool, no_push: bool, message: Option<&str>) -> anyhow::Result<()> {
    // 1. 定位仓库
    ...
    let build = Command::new("cargo").current_dir(&repo_path)
        ...
        .status()?;
    if !build.success() { anyhow::bail!("Linux 编译失败"); }
    println!("✓ Linux 编译成功");

    let linux_bin = repo_path.join("target/release/rxt");
    if !linux_bin.exists() { anyhow::bail!("Linux 产物不存在: {}", linux_bin.display()); }

    // 4. 编译 Windows (交叉编译)
    println!("\n🔨 编译 Windows (x86_64-pc-windows-gnu)...");
    let win_target = "x86_64-pc-windows-gnu";
    let win_build = Command::new("cargo").current_dir(&repo_path)
...
fn install_local(linux_bin: &PathBuf) -> anyhow::Result<()> {
    ...
    let sudo_install = Command::new("bash")
        .arg("-c")
        .arg(format!("sudo install -m 755 {} /usr/local/bin/rxt", linux_bin.display()))
        .stdout(std::process::Stdio::inherit())
...
fn deploy_xian(win_bin: &PathBuf) -> anyhow::Result<()> {
    ...
    let scp = format!("sshpass -e scp -o StrictHostKeyChecking=no {} {}@{}:rxt-new.exe",
        win_bin.display(), xian.user, xian.host);
    let scp_result = Command::new("bash").arg("-c").arg(&scp)
    ...
    match verify {
        Ok(o) if o.status.success() => {
            ...
            println!("  ✓ xian 部署成功: {}", ver.trim().lines().next().unwrap_or(""));
        }
...
fn git_publish(repo: &PathBuf, message: Option<&str>, dirty: &str) -> anyhow::Result<()> {
    ...
    // add
    let add = Command::new("git").current_dir(repo).args(["add", "-A"])
        .stdout(std::process::Stdio::piped()).stderr(std::process::Stdio::piped()).status()?;
    if !add.success() { anyhow::bail!("git add 失败"); }

    // commit
    let msg = message.unwrap_or("release: rxt publish 自动提交");
    let commit = Command::new("git").current_dir(repo)
        .args(["commit", "-m", msg])
        .stdout(std::process::Stdio::inherit()).stderr(std::process::Stdio::inherit()).status()?;
    // commit 可能因为"nothing to commit"失败, 不阻塞 push

    // push github
    println!("  push github...");
    let push_gh = Command::new("git").current_dir(repo)
    ...
    // push origin
    let push_origin = Command::new("git").current_dir(repo)
...
fn find_repo(explicit: Option<&str>) -> anyhow::Result<PathBuf> {
    ...
    let home = dirs::home_dir().unwrap_or_else(|| PathBuf::from("/tmp"));
    for c in &[home.join("projects/rxt"), home.join("code/rxt"), home.join("rxt")] {
