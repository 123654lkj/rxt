...
pub fn run(dir: &str, min_size: &str, exts: Option<&str>, delete: bool, json: bool) -> anyhow::Result<()> {
    ...
    if !root.is_dir() { anyhow::bail!("{} 不是目录", dir); }
...
/// 阶段1: 收集文件
fn collect_files(root: &Path, min_bytes: u64, ext_filter: &Option<Vec<String>>) -> (Vec<PathBuf>, u64, u64) {
    ...
    walk(root, &mut |p| {
        ...
        if let Ok(meta) = fs::metadata(p) {
            if meta.len() < min_bytes || !meta.is_file() { return; }
...
/// 输出结果 + 可选删除
fn report(duplicates: &[Vec<PathBuf>], elapsed: std::time::Duration, delete: bool, json: bool) -> anyhow::Result<()> {
    let mut waste = 0u64;
    ...
    println!("\n📋 发现 {} 组重复 ({} 个文件), 浪费 {}, 用时 {:.1}s",
        duplicates.len(), duplicates.iter().map(|g| g.len()).sum::<usize>(),
        fmt_size(waste), elapsed.as_secs_f64());
    if duplicates.is_empty() { println!("🎉 没有重复文件!"); return Ok(()); }
    for (i, group) in duplicates.iter().enumerate() {
        ...
        for (j, f) in group.iter().enumerate() {
            println!("  {} {}", if j == 0 { "✓ 保留" } else { "  删除" }, f.display());
        }
    }
    if delete {
        println!("\n⚠ --delete 模式");
        ...
        for group in duplicates {
            for f in group.iter().skip(1) {
                if let Ok(meta) = fs::metadata(f) { freed += meta.len(); }
...
fn sample_hash(path: &Path) -> anyhow::Result<String> {
    ...
    let n = f.read(&mut buf)?; hasher.update(&buf[..n]);
    if total > 12288 {
        f.seek(SeekFrom::Start(total / 2))?; let n = f.read(&mut buf)?; hasher.update(&buf[..n]);
        f.seek(SeekFrom::Start(total.saturating_sub(4096)))?; let n = f.read(&mut buf)?; hasher.update(&buf[..n]);
    }
    ...
    if let Ok(rd) = fs::read_dir(p) {
        for entry in rd.flatten() {
            ...
            if path.is_dir() {
                ...
                if name_str.starts_with('.') || matches!(&*name_str, "node_modules"|"target"|"__pycache__"|".git") { continue; }
                walk(&path, cb);
            } else { cb(&path); }
        }
    }
...
fn parse_size(s: &str) -> anyhow::Result<u64> {
    ...
    let s = s.trim();
    let (num, unit) = s.find(|c: char| !c.is_ascii_digit() && c != '.').map(|i| (&s[..i], &s[i..])).unwrap_or((s, ""));
